...
diseases_array = ['Fungal infection', 'Allergy', 'GERD', 'Chronic cholestasis', 'Drug Reaction', 'Peptic ulcer diseae', 'AIDS', 'Diabetes ', 'Gastroenteritis', 'Bronchial Asthma', 'Hypertension ', 'Migraine', 'Cervical spondylosis', 'Paralysis (brain hemorrhage)', 'Jaundice', 'Malaria', 'Chicken pox', 'Dengue', 'Typhoid', 'hepatitis A', 'Hepatitis B', 'Hepatitis C', 'Hepatitis D', 'Hepatitis E', 'Alcoholic hepatitis', 'Tuberculosis', 'Common Cold', 'Pneumonia', 'Dimorphic hemmorhoids(piles)', 'Heart attack', 'Varicose veins', 'Hypothyroidism', 'Hyperthyroidism', 'Hypoglycemia', 'Osteoarthristis', 'Arthritis', '(vertigo) Paroymsal  Positional Vertigo', 'Acne', 'Urinary tract infection', 'Psoriasis', 'Impetigo']
...
router.get("/:id", (req, res) => {
    ...
    Result.findById(objectId, (err, mres) => {
        ...
        for (var i=0; i<diseases_array.length; i++){
                ...
                mres.diseases[i] = { name: diseases_array[i], percn: mres.diseases[i], percs: (mres.diseases[i]*100).toFixed(2).toString()+"%"}
            }
        ...
        console.log(mres)
        // res.send(mres)
        res.render("result", {diseases: mres.diseases, symptoms: mres.symptoms, createdAt: mres.createdAt, user: user, pdn:mres.predictionMain[0], pdp: mres.predictionMain[1]})
    })
})
